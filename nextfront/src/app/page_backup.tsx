...
export default function Home() {
  ...
  const [selectedTab, setSelectedTab] = useState<'ALL' | 'BOOK' | 'WEB' | 'APP'>('ALL');
  const [sortType, setSortType] = useState<'LIKE' | 'VIEW' | 'RECENT'>('LIKE');
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-16 max-w-4xl mx-auto">
          ...
          <div className="flex justify-center gap-4 mb-8">
            <div className="flex items-center bg-white/80 backdrop-blur-sm rounded-full px-6 py-3 shadow-lg border border-gray-200/50">
              ...
              <span className="text-sm font-medium text-gray-700">인기 프로젝트</span>
            </div>
            <div className="flex items-center bg-white/80 backdrop-blur-sm rounded-full px-6 py-3 shadow-lg border border-gray-200/50">
              <svg className="w-5 h-5 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
              </svg>
              <span className="text-sm font-medium text-gray-700">검증된 품질</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-800 mb-4">인기 프로젝트</h2>
            <p className="text-gray-600">커뮤니티에서 가장 사랑받는 프로젝트들을 만나보세요</p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            <div className="flex bg-white rounded-xl p-2 shadow-lg border border-gray-200/50">
              {(['ALL', 'BOOK', 'WEB', 'APP'] as const).map((tab) => (
                ...
                >
                  {tab === 'ALL' ? '전체' : tab === 'BOOK' ? '북' : tab === 'WEB' ? '웹' : '앱'}
                </button>
              ))}
            </div>
            
            <div className="flex bg-white rounded-xl p-2 shadow-lg border border-gray-200/50">
              {(['LIKE', 'VIEW', 'RECENT'] as const).map((sort) => (
                <button
                  key={sort}
                ...
                >
                  {sort === 'LIKE' ? '좋아요순' : sort === 'VIEW' ? '조회순' : '최신순'}
                </button>
              ))}
            </div>
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200/50 animate-pulse">
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-800 mb-2">프로젝트를 불러올 수 없습니다</h3>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-800 mb-2">프로젝트가 없습니다</h3>
              <p className="text-gray-600">아직 등록된 프로젝트가 없어요</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {projects.map((project) => (
                <div key={project.id} className="group bg-white rounded-2xl p-6 shadow-lg border border-gray-200/50 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
                  {/* 프로젝트 이미지 */}
                    ...
                    {project.tags && project.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {project.tags.slice(0, 3).map((tag, index) => (
                          <span
                            key={index}
                            className="px-2 py-1 bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 text-xs rounded-full"
                          >
                            #{tag}
                          </span>
                        ))}
